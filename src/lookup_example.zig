...
pub fn main() !void {
    ...
    while (idx <= 7) : (idx += 1) {
        ...
        try color.colorPrint(std.io.getStdOut().writer(), "{s}", .{int_str}, fg_white, bg, false);
    }
    std.debug.print("\n", .{});
    ...
    while (idx <= 15) : (idx += 1) {
        const bg = color.Color.by_lookup(@truncate(u8, idx));
        const int_str = try std.fmt.bufPrint(int_buff[0..], "{:4}", .{idx});
        try color.colorPrint(std.io.getStdOut().writer(), "{s}", .{int_str}, fg_black, bg, false);
    }
    std.debug.print("\n", .{});

    std.debug.print(\\- 6 × 6 × 6 cube (216 colors): 16 + 36 × r + 6 × g + b (0 ≤ r, g, b ≤ 5)
